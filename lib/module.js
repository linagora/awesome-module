...
function asyncPassthrough(moduleLibs, callback) {
  callback();
}
...
AwesomeModule.prototype.getStateResult = function(state) {
  if (! (state in this.promises)) {
    return null;
  }
  if (!this.promises[state].isFulfilled()) {
    return null;
  }
...
AwesomeModule.prototype.playState = function(state, iface) {
  ...
  if (!this.settings[state]) {
    this.settings[state] = asyncPassthrough;
  }

  this.promises[state] = q.nfcall(this.settings[state], iface);
  return this.promises[state];
};
